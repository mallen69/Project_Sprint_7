...
class Facility_Risks(Base):
    ...
    wrs_pollutant_limits_id = Column(Integer, ForeignKey('wrs_pollutant_risks.id')) # pollutant limits (either NEL or other)
    wrs_pollutant_concentrations_id = Column(Integer, ForeignKey('wrs_pollutant_risks.id'))# existing pollutant concentrations from sampling data, or modeled
    wrs_pollutant_risks_id = Column(Integer, ForeignKey('wrs_pollutant_risks.id')) # WRS pollutant risks
